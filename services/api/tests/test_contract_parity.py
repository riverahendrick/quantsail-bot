...
if str(services_dir) not in sys.path:
    ...
    sys.path.insert(0, str(services_dir / "api"))
    sys.path.insert(0, str(services_dir / "engine"))
...
class TestConfigParity(unittest.TestCase):
    ...
    def test_schema_fields_match(self):
        ...
        self.assertFalse(missing_in_api, f"API config missing fields found in Engine: {missing_in_api}")
    ...
    def _compare_models(self, api_model, engine_model, path="BotConfig"):
        ...
        for name, engine_field in engine_fields.items():
            ...
            if hasattr(engine_type, "model_fields") and hasattr(api_type, "model_fields"):
                ...
                # Compare string representation of types, checking for base names to ignore module path differences
                # e.g. 'quantsail_engine.config.models.ExecutionConfig' vs 'app.schemas.config.ExecutionConfig'
                ...
                self.assertEqual(clean_type(engine_type_str), clean_type(api_type_str),
