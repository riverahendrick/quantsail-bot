...
- **Always document changes** — Update `README.md` (run/test docs) and `docs/CHANGELOG.md` (what changed + why)
...
> This appendix is embedded from your previous `TESTING_STRATEGIES.md` so **nothing is lost**. Keep it updated as you discover better tooling.
...
## 🧠 FINAL COMPLIANCE GATE (MANDATORY)

**Before you write ANY code or propose ANY plan**, your **first reply** in this task must start with the **full emoji compliance string below** (exact order, no missing emojis), on its own line:

❤️🎭🧩🛡️📋🌟🧪📝✔️✅🧠

Immediately after that line, you must include a **“Readback”** section that proves you understood the rules and the project context:

### Readback (Required)
- **Project type & stack detected:** (e.g., WEB / MOBILE / API / CRYPTO + Next.js / Expo / Node / Python, etc.)
- **Applicable rule tags:** (list the tags you will enforce for this task)
- **Top 8 non-negotiables (in your own words):** (bullet list)
- **Mocking mode:** (REAL by default; if mocks are needed, explain the gating + CI anti-leak)
- **Test/quality gates you will run before completion:** (lint, typecheck, tests, coverage threshold source)
- **Files you will check FIRST to avoid duplication:** (PLANNING.md, FEATURES.md, README, existing components/services)

❗️If you do **not** start with the emoji string + Readback, you must **stop immediately**, re-read `MASTER_GLOBAL_RULES.md`, and re-reply correctly. Do not proceed with implementation until this gate is satisfied.
